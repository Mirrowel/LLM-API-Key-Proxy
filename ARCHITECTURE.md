...
**Strategy:** One grounded classifier reads evidence in trust order — structured body fields (`status`/`code`/`type` across all four protocol vocabularies) → quota markers at any status → HTTP status → transport class — then the decision matrix maps the class to an action (rotate credential, retry same key, fail over to the next route target, abort). Free-text sniffing is limited to two narrow checks (context spellings; quota tokens at 400/429), never bare substring tokens.

- Error classification: `src/rotator_library/core/errors.py` re-exports `classify_error()`, `should_rotate_on_error()`, `should_retry_same_key()`, and the structured `StructuredAPIResponseError` envelope whose `to_protocol_payload()` renders official vocabulary per client dialect (openai chat/Responses objects, status-aware Anthropic `timeout_error`/`overloaded_error`, Gemini `google.rpc` status strings); the engine lives in `src/rotator_library/error_handler.py` (`_classify_structured_error_text()`, `_structured_quota_signal()`, `_QUOTA_MESSAGE_TOKENS`)
- Decision matrix: authentication, forbidden, not_found, and conflict fail over to the next route target instead of hard-stopping a chain; quota discriminates cooldown-worthy signals (`quotaId` PerMinute vs PerDay, spend-cap signatures) from true rate limits; refusals (content_filter/refusal/SAFETY families) never classify as errors — `src/rotator_library/routing/types.py`, `src/rotator_library/routing/policy.py`
- Fail-fast lifecycle: a stop verdict escapes the credential-loop catch-alls via `_FailDecision` in `src/rotator_library/client/executor.py`, so a deterministic client error surfaces after one attempt instead of being retried against every key; the consecutive-quota strike counter survives interleaved transient errors (`_QUOTA_STRIKE_RESET_CLASSES`), non-stream waits ask the request deadline first, a cooldown longer than the request budget returns a structured 429 (never a raw 500), and post-completion bookkeeping failures cannot destroy a finished response
- Cooldown parsing: `src/rotator_library/error_handler.py` — one duration parser handles integer seconds, Go durations, protobuf fractional/compound values, milliseconds, days, HTTP-dates, and RFC 3339 reset headers across `Retry-After`, `x-ratelimit-reset-*`, and `anthropic-ratelimit-*-reset`
- Shell error bridge: `src/proxy_app/route_helpers.py` — `classify_route_error()` / `route_error_response()` collapse the four hand-copied route exception ladders into one bridge over the library classifier and grounded status ladder, producing dialect-shaped 400s/401s/429s/5xx (malformed bodies, model routes, auth, quota-stats internals, non-boolean `stream` values) instead of 500s
...
**Secrets:** Usage state persists only derived accessor ids (`derive_accessor_id()` truncated SHA-256 in `src/rotator_library/usage/identity/registry.py`; `UsageStorage` schema v3 migrates raw accessors on load and rewrites immediately), quota-stats responses never carry raw keys, request-log redaction covers `x-goog-api-key`/`api-key` variants, error records scrub bare `key=`/`token=` query params, and `log_extra` is path-traversal guarded. The Responses WebSocket accepts `Authorization: Bearer`, `x-api-key`, `x-goog-api-key`, and `?key=` and denies with close code 1008; CORS exposes `X-Proxy-Session-Domain`.
