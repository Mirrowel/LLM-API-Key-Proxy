...
def test_bridge_adds_parent_response_messages_for_previous_response_id() -> None:
    ...
    assert kwargs["_session_tracking_hints"].global_strong_anchors == [
        "responses_previous_response_id:resp_parent"
    ]
    assert kwargs["_session_tracking_hints"].affinity_key == "responses_previous_response_id:resp_parent"
    assert kwargs["_session_tracking_hints"].session_scope is None
...
def test_bridge_preserves_chat_response_top_level_cost_with_usage() -> None:
    protocol = ResponsesProtocol()
    bridge = ResponsesBridge(protocol)
    unified = protocol.parse_request({"model": "gpt-test", "input": "Hello"})
    chat_response = {
    ...
    assert response["usage"]["cost_details"]["request_cost_usd"] == 0.012
