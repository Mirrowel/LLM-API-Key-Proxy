...
async def test_adapted_wire_feeds_cache_extract_usage_and_traces(tmp_path) -> None:
    ...
    from tests.txn_helpers import by_pass, changes

    names = [entry["pass_name"] for entry in changes(logger)]
    assert names.index("after_response_adapter_chain") < names.index("after_response_field_cache_extraction")
    assert names.index("after_response_adapter_chain") < names.index("parsed_native_unified_response")
    usage = by_pass(logger, "final_client_response")[-1]["data"]["usage"]
    assert usage["input_tokens"] == 99  # adapted wire usage won
