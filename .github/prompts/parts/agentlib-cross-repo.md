...
- **Mechanics.** If the `ACCOUNT_GH_TOKEN` environment variable is present, foreign writes go through your public account identity: `gh api` with an explicit `-H "Authorization: Bearer $ACCOUNT_GH_TOKEN"` header on the request (the ambient GH_TOKEN is home-scoped and will not work abroad). If it is absent, foreign writes are unavailable — deliver your findings as a report in the origin thread (with links) and let a human act on them.
