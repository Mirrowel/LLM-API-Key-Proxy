...
**W2 — Neutral model completeness.** Candidate identity + per-candidate stop; refusal, annotations/citations, built-in tool items; content-block index; output modality identity; provenance fields on cached state; `session_tracking._anchors_from_response` learns neutral shapes (chat parser as legacy fallback). *Acceptance: types carry them; parsers extract; builders emit or reject per D7; anchor fixture.*

**W3 — Same-protocol fidelity (D4).** Raw-preserving transport; overlay semantics for canonical edits; encrypted_content, filenames, nested shapes, unknown extensions survive; overlay trace record lists every applied overlay (this record is also W12 reconstruction's carrier). *Acceptance: byte-level fidelity fixtures modulo explicit traced overlays.*

**W4 — Cross-protocol conversion (D7/D9).** Instruction ordering nuance; reasoning-control mapping table + recorded summaries; refusal/built-in-tools/annotations; Chat `n` ↔ canonical candidate-count; first-wins; media/image, structured output, generation controls, tool-choice, canonical stop/status mapping (restored test contract); Gemini safety-settings passthrough guarantee. *Acceptance: every D7 class defined per protocol pair, fixture-covered; summaries content-asserted.*
