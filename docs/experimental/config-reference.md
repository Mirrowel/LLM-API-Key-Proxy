...
Providers declaring `protocol_name` execute natively by default (`auto`); LiteLLM runs only as an explicit, logged fallback (`litellm_fallback` execution identity in transaction metadata). `<NAME>_API_BASE` overrides a built-in provider's native transport base. `deepseek` is a pure declaration since its G8 remake (native-first, no custom path). Multi-profile providers (D13) are addressed as `provider:profile/model`. A bare name resolves to the profile matching the client protocol; when no profile speaks it, the proxy converts through the default protocol priority (`openai_chat` → `responses` → `anthropic_messages` → `gemini`, first offered wins) and emits a terminal warning naming the chosen profile. An explicit `:profile` that is unknown still fails loudly. When several profiles speak the client protocol and none is declared the default, the request errors listing the candidate profiles and suggesting a default — endpoint selection is never guessed.
