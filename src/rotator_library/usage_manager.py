...
class UsageManager:
                ...
                for key, usage in tier1_keys:
                    ...
                    async with state["lock"]:
                        if not state["models_in_use"]:
                            state["models_in_use"][model] = 1
                            lib_logger.info(
                                f"Acquired Tier 1 key {mask_credential(current_cred)} for model {model} "
                                f"(selection: {selection_method}, usage: {usage})"
                            )
                            return key
                ...
                for key, usage in tier2_keys:
                    ...
                    async with state["lock"]:
                        ...
                        if current_count < max_concurrent:
                            state["models_in_use"][model] = current_count + 1
                            lib_logger.info(
                                f"Acquired Tier 2 key {mask_credential(current_cred)} for model {model} "
                                f"(selection: {selection_method}, concurrent: {state['models_in_use'][model]}/{max_concurrent}, usage: {usage})"
                            )
                            return key
    ...
    async def release_key(self, key: str, model: str):
        ...
        async with state["lock"]:
            if model in state["models_in_use"]:
                ...
                lib_logger.info(
                    f"Released credential {mask_credential(current_cred)} from model {model} "
                    f"(remaining concurrent: {max(0, remaining)})"
                )
            else:
                lib_logger.warning(
                    f"Attempted to release credential {mask_credential(current_cred)} for model {model}, but it was not in use."
                )
    ...
    async def record_success(
        ...
        async with self._data_lock:
            ...
            if (
                ...
                lib_logger.info(
                    f"Recorded usage from response object for key {mask_credential(current_cred)}"
                )
                try:
    ...
    async def record_failure(
        ...
        async with self._data_lock:
            ...
            if classified_error.error_type in ["rate_limit", "quota_exceeded"]:
                ...
                lib_logger.info(
                    f"{classified_error.error_type} error on key {mask_credential(current_cred)} for model {model}. "
                ...
                lib_logger.warning(
                    f"Authentication error on key {mask_credential(current_cred)}. Applying 5-minute key-level lockout."
                )
            ...
            if should_increment:
                ...
                if cooldown_seconds is None:
                    ...
                    lib_logger.warning(
                        f"Failure #{count} for key {mask_credential(current_cred)} with model {model}. "
                ...
                lib_logger.info(
                    f"Provider-level error ({classified_error.error_type}) for key {mask_credential(current_cred)} with model {model}. "
            ...
            lib_logger.warning(
                f"Cooldown applied for key {mask_credential(current_cred)} with model {model}: {cooldown_seconds}s. "
                f"Error type: {classified_error.error_type}"
            )
    ...
    async def _check_key_lockout(self, key: str, key_data: Dict):
        ...
        if long_term_lockout_models >= 3:
            ...
            lib_logger.error(
                f"Key {mask_credential(current_cred)} has {long_term_lockout_models} models in long-term lockout. Applying 5-minute key-level lockout."
            )